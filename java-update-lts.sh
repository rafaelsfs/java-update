...
MAJOR=`curl -s https://www.oracle.com/java/technologies/javase-downloads.html |grep LTS |awk -F" " '{print $3}' |tail -n1`

echo $MAJOR

#Check the available version of openjdk
AVAILABLE=`curl https://www.oracle.com/java/technologies/javase-downloads.html |grep latest |grep $MAJOR |awk -F" " '{print $3}'`
...
ARQ=`curl -s https://cdn.azul.com/zulu/bin/ |grep jdk$AVAILABLE-linux_x64.tar.gz |awk -F"\"" '{print $6}' |tail -n1`

#Save download link
LINK=https://cdn.azul.com/zulu/bin/$ARQ
...
if [ "$AVAILABLE" != "$INSTALLED" ]; then
    ...
    DIR=`ls -lt |awk -F" " '/jdk/ {print $9}' |tail -n1`     
